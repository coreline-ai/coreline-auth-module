...
def test_session_touch_interval_throttles_update_session() -> None:
...
def test_session_touch_interval_zero_updates_session() -> None:
    storage = CountingMemoryAuthStorage()
    service = CorelineAuthService(
        storage=storage,
...
def test_sqlite_storage_pragmas_indexes_and_list_filters(tmp_path: Path) -> None:
    ...
    try:
        ...
        service.create_user(email="author@example.com", role=Role.AUTHOR, password="correct horse battery", email_verified=True, display_name="Author")
        ...
        assert [user.primary_email for user in storage.list_users(role=Role.AUTHOR)] == ["author@example.com"]
